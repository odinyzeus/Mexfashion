...
class FingerTemplate(models.Model):
    ...
    # attendance_device_id = fields.Many2one('attendance.device', string='Machine Device')
    ...
    user_id = fields.Char(string='ID Number', help="The ID Number of the user/employee in the machine storage",
                        related='device_user_id.user_id', store=True)
    ...
    protocol = fields.Selection(string='Protocol', related='device_id.protocol', readonly=True,
                                help="Choose the connection method to the attendance device."
                                " Please select the iCloud function if the device supports ADMS functionality.")
    size = fields.Integer(string='Size Of Finger Template')
        ...
        for device in devices:
            for user in device_users.filtered(lambda u: u.device_id == device):
                ...
                for template in self.filtered(lambda t: t.device_user_id == user and t.device_id == device):
                    finger = Finger(template.uid, template.fid, template.valid, template.template)
                    if template.protocol == 'icloud':
                        finger.size = template.size
